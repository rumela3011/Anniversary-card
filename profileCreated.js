...
if(loadList!=null){
	loadList.addEventListener('click',function(){
		window.open("listOfAnn.html","_self");
	});
}
...
var userRef = firebase.database().ref('anniversary/' + phone+"/");
var curRating = 5;
var phoneNum;
...
//	console.log(keys);
//		var key = usersData[i];
		 mrName = usersData.mrname;
		 mrsName=usersData.mrsname;
		...
		console.log(mrName,mrsName,email,phoneNum,dob,image);
		document.getElementById('nameText1').innerHTML = mrName;
		document.getElementById('nameText2').innerHTML = mrsName;
...
function onratingClicked(ratId){
	...
	if(ratId==='star5')
		...
		firebase.database().ref('anniversary/' +phone).update({
		rating:newRating
			}).then(function() {
   			 alert('Thanks for rating '+mrName+"&"+mrsName+" "+ratingGiven +" stars!");
   			 userRef.once('value',gotData , errorData);
function gotData(data){
	console.log("Hi"+phone);
	console.log(data.val());
